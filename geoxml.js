...
function GeoXml(myvar, map, url, opts) {
  ...
  if(typeof getcapproxy !="undefined") { 
	  	if(fixUrlEnd){ getcapproxy = fixUrlEnd(getcapproxy); }
	  	this.proxy = getcapproxy+"url=";
 		}
...
GeoXml.prototype.createMarker = function(point,name,desc,style,idx,instyle,visible,kml_id) {
 ...
 var html,html1,html2,html3,html4;
   html = "<div style = 'width:"+this.iwwidth+"px'>" + "<h1 "+this.titlestyle+">"+name+"</h1>" +"<div "+this.descstyle+">"+desc+"</div>";
...
if(this.opts.sidebarid){
  ...
  GEvent.addListener(p,"click", function(point, overlay) {
	if(!point && this.geoxml.opts.alwaysinfopop){
...
GeoXml.prototype.recurseJSON = function (doc, title, desc, sbid, depth){
	...
	if((!depth && doc.folders.length >1) || doc.marks.length){
...
GeoXml.addSidebar = function(myvar, name, type, e, graphic, ckd, i) {
   ...
   case  "marker" :  h = '<li id="'+mid+'" onmouseout="GEvent.trigger(' + myvar+ '.overlayman.markers['+e+'],\'mouseout\');" onmouseover="GEvent.trigger(' + myvar+ '.overlayman.markers['+e+'],\'mouseover\');" ><a href="#" onclick="GEvent.trigger(' + myvar+ '.overlayman.markers['+e+'],\'click\');return false;">'+ graphic + name + '</a></li>';
   break;
	...
	if(coordset.length <1){
 	...
 	for (var ln = 0; ln < mark.childNodes.length; ln++) {
		...
		switch(base){
			...
			case "desc":
			case "description":
			    desc = getDescription(mark.childNodes.item(ln));
			    if(!desc){ desc = nv; }
...
GeoXml.prototype.processKML = function(node, marks, title, sbid, depth) {  
			...
			case "Document" :
		 	...
		 	case "GroundOverlay":
				...
				if(url.match(rs)){
					...
					makewms = true;
					ol = this.makeWMSTileLayer(url, visible, mytitle, opacity);
					ol.bounds = wmsbounds;
					ol.title = mytitle;
					ol.opacity = opacity;
					ol.visible = visible;
	...
	if(ground){
		if (this.basesidebar) {
    			n = this.overlayman.markers.length;
    			blob = '<span style="background-color:black;border:2px solid brown;">&nbsp;&nbsp;&nbsp;&nbsp;</span> ';
		 ...
		 if(typeof fid =="number" && fid != idx){
			...
			if(!sub) { 
			       this.overlayman.folderBounds[fid] = new GLatLngBounds(); 
			        ...
			        sw = this.overlayman.folderBounds[fid].getSouthWest();
			        ne = this.overlayman.folderBounds[fid].getNorthEast();
			        this.overlayman.folderBounds[idx].extend(sw);
			        this.overlayman.folderBounds[idx].extend(ne);
			        }
...
GeoXml.prototype.processing = function(xmlDoc,title, latlon, desc, sbid) {
   ...
   if(basename == "kml") {	
	...
	if(title.length >this.maxtitlewidth){
        ...
        var style = new GIcon(G_DEFAULT_ICON,this.rssicon);
...
GPolyline.prototype.redraw = function(force) {
   ...
   var dom;
	if(!this.cursor){this.cursor = "crosshair"; }
	var shps;
...
GPolyline.prototype.setStrokeDash = function(dash) {
    ...
    if(navigator.userAgent.indexOf("MSIE") != -1){
		...
		if (this.dash == "dot") { dom.stroke.dashstyle = "dot";  }  
        else { 
...
GPolygon.prototype.redraw = function(force) {
   ...
   if(navigator.userAgent.indexOf("MSIE") != -1){
       ...
       if(shps.length){
            dom = shps[shps.length-1];//assume ours is the most recently added by the superclass redraw
            dom.style.cursor = "crosshair";//to show mouseover 
	...
	if(update_side && count>0){
		 if (clusterer.paren.progress == 0) {
     			 ...
     			 if (!clusterer.paren.opts.nozoom) {
        			//clusterer.map.setZoom(clusterer.map.getBoundsZoomLevel(clusterer.paren.bounds));
        			//clusterer.map.setCenter(clusterer.paren.bounds.getCenter());
      				}
...
GeoXml.prototype.loadJSONUrl = function (url, title, latlon, desc, idx) {
   var that = this;
   GDownloadUrl(url, function(doc) { that.parseJSON(doc,title, latlon, desc, idx); } );
   };

GeoXml.prototype.loadXMLUrl = function (url, title, latlon, desc, idx) {
	var that = this;
	that.DownloadURL(url, function(doc) {that.processing(GXml.parse(doc),title, latlon, desc, idx);},title );
	};
